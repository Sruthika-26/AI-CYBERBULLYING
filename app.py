...
def gender_based_detection(text):
    ...
    male_targets = {"boy", "man", "guy", "male"}
    female_targets = {"girl", "woman", "lady", "female"}
    female_insults = {"bitch"}
    ...
    for ent in doc.ents:
        if ent.label_ == "PERSON":
            person_names.add(ent.text.split()[0].capitalize())

    for token in doc:
        if token.pos_ == "PROPN":
            person_names.add(token.text.split()[0].capitalize())

    for token in doc:
        word = token.lemma_.lower()

        if word in female_insults:
            if token.head.ent_type_ == "PERSON":
                name = token.head.text.capitalize()
